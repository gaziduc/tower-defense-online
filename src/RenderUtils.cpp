...
void RenderUtils::render_text_shaded_prompt(SDL_Window* window, SDL_Renderer* renderer, std::string& text, SDL_FRect* dst_pos, SDL_Color fg_color, SDL_Color bg_color, float ratio_x, float ratio_y) {
    float w = render_text_shaded(window, renderer, text, dst_pos, fg_color, bg_color, ratio_x, ratio_y);
    if (SDL_GetTicks64() % 1000 < 500) {
        ...
        std::string prompt("_");
        render_text_shaded(window, renderer, prompt, dst_pos, fg_color, bg_color, ratio_x, ratio_y);
    }
}

float RenderUtils::render_text_shaded(SDL_Window* window, SDL_Renderer* renderer, std::string& text, SDL_FRect* dst_pos, SDL_Color fg_color, SDL_Color bg_color, float ratio_x, float ratio_y) {
    if (text.empty()) {
        return 0;
    }
